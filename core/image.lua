...
function image:updateAndRotImage(_image, _x, _y, _angle)
	local imageID = _image

	if imageID > 0 then
		local tableIndex = #self.propTable
		self.propTable[imageID].prop:setLoc(_x, _y)
		self.propTable[imageID].prop:setRot(_angle)
	else
		print("PAHIL IN UPDATE IMAGE")
	end
end

function image:seekLoc(_image,_x,_y)
	local imageID = _image

	if imageID > 0 then
		local tableIndex = #self.propTable
		self.propTable[imageID].prop:seekLoc(_x, _y)

	else
		print("PAHIL IN SEEK IMAGE")
	end

end
function image:updateAndColorImage(_image, _x, _y, _r, _g, _b)
	...
	if imageID > 0 then
		local tableIndex = #self.propTable
		self.propTable[imageID].prop:setLoc(_x, _y)
		self.propTable[imageID].prop:seekColor(_r,_g,_b,1,1.0)
	else
		print("PAHIL IN UPDATE IMAGE")
	end
end
...
function image:seekLoc(_image, _x, _y, _time)

	local imageID = _image
	if imageID > 0 then
		self.propTable[imageID].prop:seekLoc ( _x, _y, _time )
	else
...
function image:newText(_x, _y, _string)


	textbox = MOAITextBox.new ()
	textbox:setFont ( font )
	textbox:setString ( "Points Left: ".._string.."" )
	--
	textbox:setTextSize ( 14 )
	textbox:setRect ( _x, _y, resX, resY )
